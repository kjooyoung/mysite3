						...
						<td id="title" style="padding-left:${20*vo.depth}px">
							...
							<c:choose>
								<c:when test="${vo.replyCount ne 0}">
									<a href="${pageContext.servletContext.contextPath }/board/view/${vo.no}">${vo.title } [${vo.replyCount }]</a>
								</c:when>
								<c:otherwise>
				...
				<div class="pager">
					<ul>
						...
						<c:forEach begin="${map.pageMaker.startPage }" end="${map.pageMaker.endPage }" var="index">
							<li><a href="${pageContext.servletContext.contextPath }/board/${index}">${index }</a></li>
						</c:forEach>
